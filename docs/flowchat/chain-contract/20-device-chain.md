...
| 32 | 设备状态 | `device.status.update` | 内存 `DeviceStatusState next_status` |

| 33 | 设备空间 | `device.spatial_state.update` | 内存 `DeviceStatusState spatial_status` |

| 34 | 设备性能 | `device.performance.update` | `shared.device.status`、`shared.device.performance` |
...
| 执行序号 | `02-ordered-execution.md` 中第 32 项为 `device.status.update` |
...
| 执行序号 | `02-ordered-execution.md` 中第 33 项为 `device.spatial_state.update` |
...
| 输出集合 | `shared.device.status`、`shared.device.performance` | 按 contract 类型 | 输出必须只由本节点或显式 joint commit owner 写入。 |
...
| 执行序号 | `02-ordered-execution.md` 中第 34 项为 `device.performance.update` |
