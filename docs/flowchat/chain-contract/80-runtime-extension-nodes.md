...
运行时扩展节点不是 61 个 core chain 的一部分，但它们是 ordered execution 的合法调度项。文档必须明确它们和 core chain 的边界，避免把域运行时或 custom node 误写成 core chain。
...
### 目的
通信 dispatch 已收敛为 `communication.dispatch.update` 的实现细节：节点消费统一请求汇集后的 dispatchable outbox，生成 `shared.communication.dispatch.*`，不再作为 core chain 外的零散扩展路径。

### 运行时契约
- @pre: core chain 已提交 outbox row。
- @post: 写入 `shared.communication.dispatch.*`；必要时写入 `shared.sense.awareness_notify`。
- @invariant: dispatch 不拥有业务 outbox 的生成语义，只负责发送或调度结果；新通信需求不得继续新增专用 outbox 旁路。
