...
|---:|---|---|---|
| 70 | 特征接收 | `device.signature.receive.process` | 内存 `device_signature_result_set` |
| 71 | 信号传入 | `sense.signal.intake` | 内存 `device_received_signal_bucket` |
| 72 | 信号预处理 | `sense.signal.preprocess` | `shared.sense.signal`、可选 `shared.sense.signal_interest`、内存 `preprocessed_signal_observation_set` |
| 73 | 信号成检 | `sense.detection.from_signal` | 内存 `signal_detection_set` |
| 74 | 特征成检 | `sense.detection.from_signature` | 内存 `signature_detection_set` |
| 75 | 假警维护 | `sense.detection.artifact.update` | 内存 `artifact_detection_set` |
| 76 | 探测更新 | `sense.detection.update` | `shared.sense.detection`、内存 `postprocessed_detection_set` |
| 77 | 探测成观 | `sense.observation.from_detection` | 内存 `observation_candidate_set` |
| 78 | 观测更新 | `sense.observation.update` | `shared.sense.observation` |
| 79 | 航迹更新 | `sense.track.update` | `shared.sense.track` |
| 80 | 态势融合 | `sense.awareness.update` | 内存 `awareness_working_set` |
| 81 | 态势维护 | `sense.awareness.maintain` | `shared.sense.awareness` |
...
### 目的
按接收设备独立执行信号传入判定，形成接收桶。迁移期同时接收新 `DigitizedSignalObservable` 和兼容 `PropagatedLocalSignalCandidate`。
...
| 执行序号 | 71 |
| 阶段 | Post-commit core chain |
| 上游 | `device.signature.receive.process` |
| 下游 | `sense.signal.preprocess` |

### 输入
| Contract | 来源 | 是否必需 | 说明 |
|---|---|---|---|
| 输入集合 | `runtime.signal.digitized_observables`、`runtime.environment.signal.propagated_local_candidates`、设备状态、设备性能、平台姿态 | 是 | 优先消费 receiver-bound 数字化 observable，同时保留 candidate 兼容输入。 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 71 项为 `sense.signal.intake` |
...
### 状态与保留
维护 `sense_signal_state_`；预处理集合仅当前步传给 detection。来自 digitized observable 的 provenance 写入 `source_observable_ids`，不得混入 `source_candidate_ids`。
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 72 项为 `sense.signal.preprocess` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 73 项为 `sense.detection.from_signal` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 75 项为 `sense.detection.artifact.update` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
### 状态与保留
维护 `sense_detection_state_`；postprocessed set 当前步传给 observation。observable-backed detection 保留 `source_observable_ids`，candidate-backed detection 保留 `source_candidate_ids`。
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 76 项为 `sense.detection.update` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 77 项为 `sense.observation.from_detection` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 78 项为 `sense.observation.update` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 79 项为 `sense.track.update` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 80 项为 `sense.awareness.update` |
| Build | `xmake build test-l3-unified-entry` 通过 |
...
| 检查项 | 期望结果 |
|---|---|
| 执行序号 | `02-ordered-execution.md` 中第 81 项为 `sense.awareness.maintain` |
| Build | `xmake build test-l3-unified-entry` 通过 |
