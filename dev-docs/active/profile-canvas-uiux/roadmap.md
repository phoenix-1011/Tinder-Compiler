...
| C8 | Drag semantics | **Frozen:** Standard resources are **not draggable** on the canvas — their adjustments go through C24 paths (pin / activate / candidate selection). For customs, the **library's draggable unit is the custom node (not the custom resource)** — see C26 for the resource → branch → node library hierarchy. Custom resources / branches themselves are not draggable; only their inner nodes are. The only valid drop target for a node drag is an edge of the directional flow line (see C22, C23). | Standard resources self-declare coverage via `compute_nodes[]`, so dragging them to a slot was always semantically misleading. Custom resources contain multiple nodes that each need independent placement, so the draggable unit must be the node, not the resource. |
...
| C22 | Custom usage position model | **Frozen:** Custom usages on the canvas are first-class **nodes** alongside slot nodes — not attached to slots, not decorations on a line. Each canvas custom node corresponds to one entry in `profile.custom_node_usages`, which is a **node-level placement** decoupled from resource-family membership (C26). The execution flow is a sequence of canonical-order edges: `[slot]→[custom]→[custom]→[slot]→…`. Position is determined by `anchorChainId` + `arrayIndex` (anchor identifies the inter-slot interval; `arrayIndex` orders multiple customs within that interval). The same library node may be placed multiple times — each placement is an independent usage with its own anchor / arrayIndex / enabled. Hiding a slot collapses the two adjacent intervals; the customs on either side preserve their `arrayIndex` order in the merged interval. | Treats customs as proper graph nodes; aligns the canvas-side placement (usage) with the library-side draggable unit (node); enables multi-placement of the same node. |
...
| C25 | Library pin metaphor | **Frozen:** Library branch rows support a `pin` action scoped to the currently selected profile. Pinning a branch sets that family's slot in the profile to `selected_branch_id = pinned_branch` and `enabled = true`. Pinning another branch of the same family transfers the pin (B8 radio behaviour). Unpinning removes the family slot from `profile.resources` entirely (deactivation alone is via C24b); for custom families, unpin also **cascades to remove all `custom_node_usages` belonging to nodes of that family**, with a one-time confirm dialog listing the affected placements. Library renders three pin states: unpinned (default), pinned + active (filled icon, highlighted row), pinned + disabled (outlined icon, highlighted but muted row). The pin context menu offers `unpin`, `停用 / 激活`, `移到目录…`, and the existing branch operations. | Collapses "add + select + activate" into one motion, gives the library a built-in view of "which branches this profile uses", and visually enforces B8's one-family-per-profile rule. The cascade ensures `custom_node_usages` cannot point at a resource not in `profile.resources`. |
| C26 | Custom node placement model | **Frozen:** The library exposes custom resources as a **3-level hierarchy** — family → branch → custom node — and the **node row is the draggable unit** (not the family, not the branch). Pin remains at the branch level (C25) and governs only resource-family membership + `enabled` state; it does not auto-place any nodes. Dragging a node from an unpinned branch **auto-pins that branch first** (one-step pin + placement). The same node may be placed multiple times by repeated drag — each drag creates an independent `custom_node_usages` entry. The library node row shows `·N` indicating the number of usages in the currently selected profile (`·0` rendered muted to mean "未放置"); clicking `·N` highlights the matching usages on the canvas. **Branch transfer (same-family pin transfer)** uses a soft-orphan policy: usages whose `node_id` exists in the new branch are preserved; usages with missing `node_id` are marked `孤立 usage` (red border + `⚠ 在分支 X 中不存在`) on the canvas, and a confirm dialog listing soon-to-be-orphans is shown before the transfer commits. Standard library remains 2-level (family → branch); standard nodes are never draggable per C8. | A custom resource contains multiple independently-invocable nodes with no auto-placement; treating the resource (or branch) as the draggable unit would conflate membership with placement. The 3-level library + node-level drag aligns the library UX with the `custom_nodes[]` data model. Auto-pin minimises clicks for the common "I want to use this custom" workflow; soft-orphan policy preserves user work across reasonable branch switches without forcing premature decisions. |
...
- library panel: `计算实例 / 标准` is 2-level (family → branches); `计算实例 / 自定义` is 3-level (family → branches → nodes, per C26)
- library shows pin state indicators (read-only in this phase — three visual states from C25, but no interactive pinning yet)
- library custom node rows show `·N` usage count for the currently selected profile (read-only highlight, click-to-highlight wired in Phase 3)
...
- library node → flow-line-edge drag for custom nodes (the only canvas drag — C8 / C22 / C26)
- auto-pin on first drag from an unpinned branch (C26)
- soft-orphan handling on branch transfer with pre-transfer confirm dialog (C26)
- canvas-side custom-node drag to reposition (reorder within and across intervals)
